        ...
        <div className="relative">
          ...
          <ul className="space-y-12 md:space-y-16">
            {careerTimeline.map((item, index) => (
              ...
              >
                {/* Dot on the line */}
                <div className="absolute left-0 md:left-1/2 w-3 h-3 -ml-1.5 mt-2 rounded-full bg-accent-500 dark:bg-accent-400 border-2 border-white dark:border-primary-950 shrink-0 z-10" />
                ...
                >
                  {item.period && (
                    <span className="text-xs font-medium text-accent-600 dark:text-accent-400/90 uppercase tracking-wider">
                      {item.period}
                    </span>
                  )}
                  <h3 className="text-lg font-semibold text-slate-900 dark:text-white mt-1">{item.title}</h3>
                  <p className="text-sm text-slate-600 dark:text-zinc-400 mt-0.5">{item.subtitle}</p>
                  {item.description && (
                    <p className="text-sm text-slate-500 dark:text-zinc-500 mt-2">{item.description}</p>
                  )}
